export function periodComparison(
  ...
  const increaseColor = options.lowerIsBetter ? '#fbbf24' : '#60a5fa';
  const decreaseColor = options.lowerIsBetter ? '#34d399' : '#fbbf24';
  ...
  const color = amount === 0 ? neutral : change < 0 ? decreaseColor : increaseColor;
  return {
    text: `${direction}${amount.toLocaleString('en-US', { maximumFractionDigits: options.percentagePoints ? 2 : 1 })}${unit}`,
    color,
