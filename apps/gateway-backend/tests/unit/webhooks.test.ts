...
test('getWebhook returns WEBHOOK_NOT_FOUND as a value', async () => {
  database.script(rows()); // the scoped select matches nothing
...
test('updateWebhook returns WEBHOOK_NOT_FOUND as a value', async () => {
  database.script(rows()); // the scoped update matches nothing
...
test('deleteWebhook returns WEBHOOK_NOT_FOUND as a value', async () => {
  database.script(rows()); // the scoped delete matches nothing
...
// whatever the test scripted, whatever the predicate said, so a passing
...
test('getWebhook rejects when the row will not parse', async () => {
  database.script(rows({ id: 'not-a-uuid' }));
