...
describe('listPrompts', () => {
  test('trims the probe row and reports more data behind it', async () => {
    ...
    database.respondTo(
      'select',
      'prompts',
      rows(promptRow({ id: ID_C }), promptRow({ id: ID_B }), promptRow({ id: ID_A })),
    );
  ...
  test('an exactly full page does not claim more data', async () => {
    database.respondTo('select', 'prompts', rows(promptRow({ id: ID_C }), promptRow({ id: ID_B })));
...
describe('createPrompt', () => {
  test('returns the created row and writes one audit entry inside the transaction', async () => {
    database.respondTo('insert', 'prompts', rows(promptRow()));
    ...
    expect(auditWrites.calls[0]?.body.event).toBe('prompts.created');
  ...
  test('a taken name is a refusal, not a crash', async () => {
    ...
    // name arrives here.
    database.respondTo('insert', 'prompts', rows());
...
describe('deletePrompt', () => {
  test('records what was deleted, since the row will not survive to be read', async () => {
    database.respondTo('delete', 'prompts', rows(promptRow()));

    expectOk(await asCaller(() => Services.deletePrompt(PROMPT_ID)));

    expect(auditWrites.calls).toHaveLength(1);
    expect(auditWrites.calls[0]?.body.event).toBe('prompts.deleted');
    // Stated rather than diffed: the audit entry is the only remaining record.
    expect(auditWrites.calls[0]?.body.metadata).toMatchObject({
      name: 'support-triage',
      active_version: 3,
    });
  });
  ...
  test('deleting something absent is a refusal with no audit entry', async () => {
    database.respondTo('delete', 'prompts', rows());
...
describe('getPromptVersion', () => {
  test('returns the version through its parent', async () => {
    ...
    database.respondTo('select', 'prompt_versions', rows({ version: versionRow() }));
