...
import { type Caller, getActorId } from '@repo/hono';
 ...
 *
 * @param caller
 * Who is performing the action being recorded.
 *
 * @param input
...
async function createAuditLog(
  ...
  const parsedBody = Schemas.createAuditLog.body.parse(body);
  ...
  const parsed = Schemas.createAuditLog.response.parse(inserted[0]);
  return parsed;
}
 ...
 *
 * @param caller
 * The authenticated caller whose organization scopes the query.
 *
 * @param id
 ...
 */
async function getAuditLog(caller: Caller, id: string): Promise<Result<GetAuditLogResponse, GetAuditLogFailure>> {
 ...
 */
async function listAuditLogs(caller: Caller, query: ListAuditLogsQuery): Promise<ListAuditLogsResponse> {
  const conditions = [
    ...
    // actor_id is interpreted against actor_type: a key id for 'api_key' rows,
    // a user id for 'user' rows. Pair the two filters to disambiguate.
