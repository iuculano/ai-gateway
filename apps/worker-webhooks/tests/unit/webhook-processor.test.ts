...
test('does nothing when the outbox is empty', async () => {
  ...
  expect(database.queriesFor('delete', 'webhook_outbox')).toHaveLength(0);
});

test('claims, posts, and records a successful delivery', async () => {
  database.respondTo('select', 'webhook_outbox', rows(claimed()));
  database.respondTo('delete', 'webhook_outbox', rows());
  database.respondTo('insert', 'webhook_deliveries', rows());
  ...
  const insert = database.queriesFor('insert', 'webhook_deliveries').at(-1);
...
test('one failed endpoint does not prevent the rest of the claimed batch from being delivered', async () => {
  ...
  database.respondTo('delete', 'webhook_outbox', rows());
  database.respondTo('insert', 'webhook_deliveries', rows(), rows());
  ...
  const attempts = database
    .queriesFor('insert', 'webhook_deliveries')
    .map((query) => query.calls.find((call) => call.method === 'values')?.args[0] as { status_code: number });
...
test('a failure to write delivery history is contained within that delivery', async () => {
  database.respondTo('select', 'webhook_outbox', rows(claimed()));
  database.respondTo('delete', 'webhook_outbox', rows());
  database.respondTo('insert', 'webhook_deliveries', failsWith(new Error('database unavailable')));
